...
#ifdef __GNUC__
/* Con GCC/STM32CubeIDE, la función que redirige printf es __io_putchar */
...
// SHM_N=512, K=8, solapamiento=50% → 2304 muestras × 4 bytes = ~9 KB */

#define BUFFER_LEN 2304
static float buffer_muestras[BUFFER_LEN];
...
static uint32_t idx_muestra = 0;

// Timestamp para muestreo preciso
static uint32_t tick_ultimo = 0;

// Intervalo entre muestras en ms según ODR real del sensor
// LIS3DSH configurado a 100 Hz → 10 ms entre muestras */
#define INTERVALO_MUESTREO_MS  10
...
PUTCHAR_PROTOTYPE
{
	// Ahora usamos huart1 que corresponde a PA9/PA10
...
int main(void)
{
	...
	MX_I2C1_Init();
	/* USER CODE BEGIN 2 */
	printf("--- Sismografo Popayan: LIS3DSH ---\r\n");
	HAL_Delay(10);

	if (LIS3DSH_Init(&hi2c1)) {
		printf("Sensor LIS3DSH OK (WHO_AM_I: 0x3F)\r\n");
	} else {
		printf("ERROR: Sensor no encontrado\r\n");
	}
	/* Inicializar línea base como inválida */
	memset(&linea_base, 0, sizeof(linea_base));
	linea_base.valida = 0;
